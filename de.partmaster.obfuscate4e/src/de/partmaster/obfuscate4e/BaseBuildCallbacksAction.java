...
public class BaseBuildCallbacksAction implements IObjectActionDelegate {
    ...
    public void run(IAction action) {
            ...
            public void run(IProgressMonitor monitor) {
                IWorkspaceRunnable wop = new IWorkspaceRunnable() {
                    ...
                    public void run(IProgressMonitor progressMonitor) throws CoreException {
                        BuildCallbacksFileUpdater fileUpdater = new BuildCallbacksFileUpdater(itsBuildGenerator,
                                fManifestFile);
                        if (fileUpdater.exists()) {
                            IFile file = fileUpdater.getFile();
                            if (MessageDialog.openConfirm(itsTargetPart.getSite().getShell(), "Confirm", file
                                    .getFullPath()
                                    + " exists, overwrite?")) {
                        ...
                        BuildCallbacksPropertyUpdater propertyUpdater = new BuildCallbacksPropertyUpdater(fManifestFile);
                        if (propertyUpdater.existsKey()) {
                            if (MessageDialog.openConfirm(itsTargetPart.getSite().getShell(), "Confirm",
                                    "Build property key exists: " + propertyUpdater.getKey() + "\nOverwrite?")) {
        ...
        try {
            PlatformUI.getWorkbench().getProgressService().runInUI(Activator.getActiveWorkbenchWindow(), op,
